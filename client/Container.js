class Container extends React.Component{
    ...
    {
      const response = await fetch('/book_shop/books' , 
                {method:'GET',  
    ...
    {
      const response = await fetch('/book_shop/authors' , 
                {method:'GET',  
                   headers: { 'Content-Type': 'application/json'}
                 });
      if ( response.status != 200 ) {
    ...
    callBookPage(book_id){
        ...
        console.log("Selected book name: " + curr_book.name +
        "Selected author name: " + book_author.name);
    ...
    renderBookPage(){
        return <BookPage MoveToAuthorPage = {this.callAuthorPage} selected_book = {this.state.selected_book} 
        callAddBookToCart = {this.callAddBookToCart}></BookPage>
    }

    renderHomePage(){
        return <HomePage onCallBook = {this.callBookPage} books = {this.state.books_arr}></HomePage>
    }

    renderAuthorPage(){
        return <AuthorPage currAuthor = {this.state.selected_book_author}  
        currAuthorBooks = {this.state.selected_author_books} 
        onCallBook = {this.callBookPage}></AuthorPage>
    }

    renderShoppingCartPage(){
        return <ShoppingCartPage shoppingCartArr = {this.state.shopping_cart_arr} onCallBook = {this.callBookPage}></ShoppingCartPage>
    }
	...
	render() {
        ...
        <div className = 'Container'>
            ...
            <div className = 'nav_bar'>
                <button className = "btn" type="button" onClick={this.callHomePage}>Home page</button>
                <button className = "btn" type="button" onClick={this.callCartPage}>Shopping Cart</button>
            </div>
            <br/>
            {this.renderCurrPage()}
		</div>
